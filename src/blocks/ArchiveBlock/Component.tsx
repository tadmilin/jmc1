...
const ActionButtons = ({ colorTheme = 'light' }: { colorTheme?: string }) => {
  ...
  return (
    <div className="flex justify-center gap-6 mt-8 flex-wrap px-4">
      <button
        onClick={() => (window.location.href = '/categories/iron')}
        className={`group flex items-center justify-center px-8 py-4 rounded-full font-semibold text-lg transition-all duration-300 transform hover:scale-105 hover:shadow-xl min-w-[200px] cursor-pointer ${
          isDarkTheme
...
export const ArchiveBlock: React.FC<ArchiveBlockProps & { id?: string; colorTheme?: string }> = (
  props,
) => {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4">
        {/* Header Section */}
        <div
          className={`text-center mb-16 max-w-4xl mx-auto ${containerBg} rounded-2xl shadow-xl border ${borderColor} overflow-hidden`}
        >
          <div
            className={`px-8 py-6 bg-gradient-to-r ${isDarkTheme ? 'from-gray-700 to-gray-600' : 'from-blue-50 to-indigo-50'} border-b ${borderColor}`}
          >
            {introContent && (
