        ...
        return (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-4">
            {features.map((feature, index) => (
              <div
                key={index}
                className={`feature-item flex flex-col items-center text-center p-4 md:p-6 rounded-lg ${colorTheme === 'dark' ? 'bg-gray-800 text-white border border-gray-700' : 'bg-white border border-blue-100'} shadow-sm ${getAnimationClass(index)}`}
              >
                <div
                  className={`w-12 h-12 md:w-16 md:h-16 mb-3 md:mb-4 rounded-full flex items-center justify-center p-2 md:p-3 ${colorTheme === 'dark' ? 'bg-blue-600 text-white' : 'text-blue-600 bg-blue-50 border-2 border-blue-200'}`}
                >
                ...
                <h3
                  className={`text-sm md:text-lg font-bold mb-1 md:mb-2 ${colorTheme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}
                >
                  {feature.title}
                </h3>
                {feature.description && (
                  <p
                    className={`text-xs md:text-sm ${colorTheme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}
                  >
                    {feature.description}
                  </p>
                )}
          ...
          <div className="flex flex-wrap justify-center items-center">
            {features.map((feature, index) => (
              ...
              >
                ...
                <h3
                  className={`text-base font-bold uppercase mb-1 ${colorTheme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}
                >
                  {feature.title}
                </h3>
                {feature.description && (
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            {features.map((feature, index) => (
              <div
                key={index}
                className={`feature-item bg-white rounded-xl shadow-lg overflow-hidden ${getAnimationClass(index)}`}
              >
                ...
                <div className="p-5">
                  ...
                  <h3
                    className={`text-lg font-bold mb-2 ${colorTheme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}
                  >
                    {feature.title}
                  </h3>
                  {feature.description && (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2 md:gap-4">
            {features.map((feature, index) => (
              ...
              >
                ...
                <div>
                  <h3
                    className={`text-sm font-semibold ${colorTheme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}
                  >
                    {feature.title}
                  </h3>
                  {feature.description && (
      ...
      <div className="container mx-auto px-4">
        {(title || subtitle) && (
          <div className="text-center mb-8">
            {title && (
              <h2
                className={`text-2xl md:text-3xl font-bold mb-3 ${colorTheme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}
              >
                {title}
              </h2>
            )}
            {subtitle && (
              <p
                className={`max-w-2xl mx-auto text-sm md:text-base ${colorTheme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}
              >
                {subtitle}
              </p>
            )}
        ...
        <div ref={featuresRef}>{renderFeatures()}</div>
      </div>
