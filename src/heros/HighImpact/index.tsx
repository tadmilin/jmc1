...
import type { Page, Category, Media as MediaType, Post /*HeroBlock, Link as LinkType*/ } from '@/payload-types'
...
const FramedContentRenderer: React.FC<{ blocks: SpecificHeroBlock[] | null | undefined }> = ({ blocks }) => {
...
const HeroActionSlotsRenderer: React.FC<{ 
  ...
  const titleColor = isDarkTheme ? 'text-gray-100' : 'text-gray-800';
  const descColor = isDarkTheme ? 'text-gray-400' : 'text-gray-600';
    ...
    <div className="w-full pt-0 pb-2 md:pb-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        {slots.map((slot) => {
          ...
          const slotContent = (
            <div className={`flex items-center space-x-4 p-4 md:p-5 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 ${cardBgColor} border ${cardBorderColor} h-full cursor-pointer`}>
              ...
              <div className="flex-grow">
                <h4 className={`font-semibold text-lg md:text-xl mb-1 ${titleColor}`}>{slot.title}</h4>
                {slot.description && (
          ...
          const isValidLink = cmsLinkProps && (
...
export const HighImpactHero: React.FC<Page['hero']> = ({ 
  ...
  const CategoriesDropdown = () => {
    ...
    const baseTextColor = colorTheme === 'dark' ? 'text-gray-300' : 'text-gray-700';
    ...
    const headerTextColor = colorTheme === 'dark' ? 'text-gray-100' : 'text-gray-800';
    const scrollbarClasses = colorTheme === 'dark' 
  ...
  return (
      <div className={`w-full max-w-sm bg-white dark:bg-gray-800 rounded-xl shadow-lg border ${borderColor} overflow-hidden`}>
        <div className={`px-4 py-3 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-700 dark:to-gray-600 border-b ${borderColor}`}>
  ...
  const SocialMediaButtons = () => {
    ...
    return (
      <div className={`w-full max-w-sm ${containerBg} rounded-xl shadow-lg border ${borderColor} overflow-hidden mt-4`}>
        <div className={`px-4 py-3 bg-gradient-to-r from-green-50 to-blue-50 dark:from-gray-700 dark:to-gray-600 border-b ${borderColor}`}>
          <h3 className={`text-lg font-semibold ${headerTextColor} flex items-center gap-2`}>
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="p-4">
          <div className="space-y-3">
            {(socialMediaButtons as SocialMediaButton[]).map((button) => (
              ...
              >
                {button.icon && typeof button.icon === 'object' && (button.icon as MediaType).url && (
    ...
    if (hasSlideImages) {
      return (
        <div className="relative rounded-xl overflow-hidden shadow-2xl w-full mb-2 aspect-video md:aspect-[16/9]">
             ...
             <div className={`absolute -right-3 -bottom-3 py-2 px-4 rounded-md flex items-center justify-center text-white shadow-lg ${colorTheme === 'dark' ? 'bg-blue-500' : 'bg-blue-600'}`}>
    ...
    >
      {backgroundImage && typeof backgroundImage === 'object' && (
        <div className="absolute inset-0 z-0">
          <Media resource={backgroundImage as MediaType} fill imgClassName={`object-cover ${colorTheme === 'dark' ? 'opacity-10' : 'opacity-20'}`} priority />
        </div>
      )}
      ...
      <div className={`container mx-auto px-1 pt-1 pb-1 md:pb-2 lg:pb-2 z-3 relative w-full`}>
        {isCentered ? (
          <div className="text-center max-w-5xl mx-auto flex flex-col">
            <MainMediaArea />
            <HeroActionSlotsRenderer slots={heroActionSlots as HeroActionSlot[]} colorTheme={colorTheme ?? 'light'} />
            <div className="space-y-4 mt-4">
              {richText && <RichText className={`text-4xl md:text-5xl lg:text-6xl font-bold bg-gradient-to-r ${colorTheme === 'dark' ? 'from-blue-300 to-indigo-300' : 'from-blue-600 to-indigo-600'} bg-clip-text text-transparent`} data={richText} enableGutter={false} />}
          {Array.isArray(links) && links.length > 0 && (
                <ul className="flex flex-wrap justify-center gap-4 mt-6">
                  {links.map(({ link }, i) => (
                    <li key={i}><CMSLink {...link} className={`px-6 py-3 rounded-full text-white font-medium transform transition-all duration-300 hover:scale-105 hover:shadow-lg inline-block ${colorTheme === 'dark' ? 'bg-blue-600 hover:bg-blue-500' : 'bg-blue-700 hover:bg-blue-600'}`} /></li>
                  ))}
            </ul>
          )}
        </div>
          ...
          <div className={`grid grid-cols-1 ${showCategoriesDropdown ? (displayFrame ? 'lg:grid-cols-[280px_1fr_320px]' : 'lg:grid-cols-[280px_1fr]') : (displayFrame ? 'lg:grid-cols-[1fr_320px]' : 'lg:grid-cols-1')} gap-8 lg:gap-12 items-start`}>
            {/* Categories Dropdown และ Social Media Buttons - แสดงเฉพาะ desktop */}
            {showCategoriesDropdown && (
              <div className={`w-full ${isReversed && displayFrame ? 'lg:order-2' : 'lg:order-1'} hidden lg:block`}>
                <CategoriesDropdown />
                <SocialMediaButtons />
                                </div>
                              )}
            ...
            `}>
              ...
              <div className="w-full px-4 md:px-0 mt-2">
                 {richText && <RichText className={`text-3xl md:text-4xl lg:text-5xl font-bold leading-tight bg-gradient-to-r ${colorTheme === 'dark' ? 'from-blue-300 to-indigo-300' : 'from-blue-600 to-indigo-600'} bg-clip-text text-transparent`} data={richText} enableGutter={false} />}
                </div>
              {Array.isArray(links) && links.length > 0 && (
                <ul className="flex flex-wrap gap-3 pt-2 justify-center lg:justify-start px-4 md:px-0 mt-2">
                  {links.map(({ link }, i) => (
                    <li key={i}><CMSLink {...link} className={`px-5 py-2.5 rounded-md text-sm font-medium transition-all duration-300 hover:shadow-lg ${colorTheme === 'dark' ? 'bg-blue-600 hover:bg-blue-500 text-white' : 'bg-blue-700 hover:bg-blue-600 text-white'}`} /></li>
                  ))}
                </ul>
              )}
            </div>
            ...
            {displayFrame && (
              <div className={`${isReversed ? (showCategoriesDropdown ? 'lg:order-1' : 'lg:order-1') : (showCategoriesDropdown ? 'lg:order-3' : 'lg:order-2')} w-full self-start lg:max-h-[calc(80vh)] min-h-[300px]`}>
